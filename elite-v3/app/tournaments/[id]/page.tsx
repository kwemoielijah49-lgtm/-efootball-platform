...
export default async function TournamentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const maxRound = allMatches.reduce((max: number, m: any) => Math.max(max, m.round), 0)
    ...
    <div className="min-h-screen bg-pitch">
      ...
      <main className="md:pl-56 pb-20 md:pb-0">
        <div className="bg-pitch-mid border-b border-pitch-border px-4 py-6">
          ...
          <h1 className="text-3xl md:text-4xl text-chalk font-bold mb-2" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{tournament.name}</h1>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <span className="text-ash text-xs flex items-center gap-1"><Calendar size={11} />{tournament.start_date ? new Date(tournament.start_date).toLocaleDateString('en-KE', { weekday: 'short', day: 'numeric', month: 'long' }) : 'Date TBD'}</span>
            <span className="text-ash text-xs flex items-center gap-1"><Users size={11} /> {(entries ?? []).length}/{tournament.max_participants}</span>
          </div>
          {tournament.prize_info && <p className="text-volt font-semibold text-sm mt-3">🏆 {tournament.prize_info}</p>}
        </div>
        ...
        <div className="px-4 py-5 max-w-2xl">
          <div className="mb-6">
            ...
            {(entries ?? []).length === 0 ? (
              <div className="card text-center py-6"><p className="text-ash text-sm">No teams registered yet.</p></div>
            ) : (
                ...
                {(entries ?? []).map((e: any) => (
                  <div key={e.id} className="card py-3 flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-pitch border border-pitch-border flex items-center justify-center flex-shrink-0">
                      <span className="text-volt font-bold text-sm" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{e.teams?.name?.[0]?.toUpperCase()}</span>
                    </div>
                    <span className="text-chalk text-sm truncate" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{e.teams?.name}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          {allMatches.length > 0 && (
            <div>
              ...
              <div className="space-y-5">
                {rounds.map((round) => (
                  <div key={round}>
                    <p className="text-volt text-xs uppercase tracking-widest mb-2" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{getRoundName(round, maxRound)}</p>
                    <div className="space-y-2">
                      {allMatches.filter((m: any) => m.round === round).map((match: any) => (
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between">
                                <span className={`text-sm ${match.winner_id === match.team_a?.id && match.status === 'confirmed' ? 'text-volt' : 'text-chalk'}`} style={{fontFamily:'Barlow Condensed,sans-serif'}}>{match.team_a?.name ?? 'TBD'}</span>
                              ...
                              <div className="flex items-center justify-between">
                                <span className={`text-sm ${match.winner_id === match.team_b?.id && match.status === 'confirmed' ? 'text-volt' : 'text-chalk'}`} style={{fontFamily:'Barlow Condensed,sans-serif'}}>{match.team_b?.name ?? 'TBD'}</span>
