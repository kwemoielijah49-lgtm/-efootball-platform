  ...
  return (
    <div className="min-h-screen bg-pitch">
      ...
      <main className="md:pl-56 pb-20 md:pb-0 px-4 py-6 max-w-2xl">
        ...
        <h1 className="text-3xl font-bold text-chalk mb-6" style={{fontFamily:'Barlow Condensed,sans-serif'}}>TOURNAMENTS</h1>

        {active.length > 0 && (
          <div className="mb-6">
            <p className="section-label mb-3 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-volt animate-pulse" /> Live Now</p>
            <div className="flex flex-col gap-3">{active.map((t: any) => <TCard key={t.id} t={t} />)}</div>
          </div>
        )}
        ...
        {upcoming.length > 0 && (
          <div className="mb-6">
            ...
            <div className="flex flex-col gap-3">{upcoming.map((t: any) => <TCard key={t.id} t={t} />)}</div>
          </div>
        )}
        ...
        {completed.length > 0 && (
          <div>
            ...
            <div className="flex flex-col gap-3">{completed.map((t: any) => <TCard key={t.id} t={t} />)}</div>
          </div>
        )}
      </main>
    </div>
    ...
    <Link href={`/tournaments/${t.id}`} className="card hover:border-volt/40 transition-colors group">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <h3 className="text-xl text-chalk group-hover:text-volt transition-colors truncate" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{t.name}</h3>
          {t.description && <p className="text-ash text-xs mt-1 line-clamp-2">{t.description}</p>}
          <div className="flex items-center gap-4 mt-2">
            <span className="text-ash text-xs flex items-center gap-1"><Calendar size={11} />{t.start_date ? new Date(t.start_date).toLocaleDateString('en-KE', { day: 'numeric', month: 'short' }) : 'TBD'}</span>
            <span className="text-ash text-xs flex items-center gap-1"><Users size={11} /> Max {t.max_participants}</span>
          </div>
        </div>
