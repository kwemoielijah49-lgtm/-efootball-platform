      ...
      <main className="md:pl-56 pb-20 md:pb-0">
        <div className="bg-pitch-mid border-b border-pitch-border px-4 py-6">
          <div className="w-16 h-16 rounded-full bg-pitch border border-pitch-border flex items-center justify-center mb-4">
            <span className="text-volt font-bold text-3xl" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{team.name[0].toUpperCase()}</span>
          </div>
          <h1 className="text-3xl text-chalk font-bold" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{team.name}</h1>
              ...
              {(members ?? []).map((m: any) => (
                <div key={m.id} className="card py-3 flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-pitch border border-pitch-border flex items-center justify-center flex-shrink-0">
                    <span className="text-volt font-bold" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{m.users?.username?.[0]?.toUpperCase()}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-chalk text-sm" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{m.users?.username}</p>
              ...
              <div className="flex flex-col gap-2">
                {(entries ?? []).map((e: any) => (
                  <div key={e.id} className="card py-3 flex items-center justify-between">
                    <p className="text-chalk text-sm" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{e.tournaments?.name}</p>
