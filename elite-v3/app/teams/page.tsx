...
export default async function TeamsPage() {
  const { data: teams } = await supabase.from('teams').select('*, captain:captain_id(username), team_members(count)').order('created_at', { ascending: false })
  const all = teams ?? []
      ...
      <main className="md:pl-56 pb-20 md:pb-0 px-4 py-6 max-w-2xl">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <h1 className="text-3xl font-bold text-chalk" style={{fontFamily:'Barlow Condensed,sans-serif'}}>TEAMS</h1>
          </div>
          <Link href="/teams/create" className="btn-primary"><Plus size={14} /> Create</Link>
        </div>
        {all.length === 0 ? (
            ...
            {all.map((team: any) => (
              <Link key={team.id} href={`/teams/${team.id}`} className="card hover:border-volt/40 transition-colors group">
                <div className="w-12 h-12 rounded-full bg-pitch border border-pitch-border flex items-center justify-center mb-3">
                  <span className="text-volt font-bold text-xl" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{team.name[0].toUpperCase()}</span>
                </div>
                <h3 className="text-lg text-chalk group-hover:text-volt transition-colors truncate" style={{fontFamily:'Barlow Condensed,sans-serif'}}>{team.name}</h3>
