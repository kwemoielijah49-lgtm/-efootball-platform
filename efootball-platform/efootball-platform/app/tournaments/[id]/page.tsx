          ...
          <div className="mb-6">
            ...
            {entries.length === 0 ? (
              <div className="card text-center py-6">
                ...
                <button className="btn-primary mt-4 mx-auto">Register Your Team</button>
              </div>
            ) : (
          ...
          {matches.length > 0 && (
            <div>
              ...
              <div className="space-y-5">
                {rounds.map((round, ri) => (
                  <div key={round}>
                    <p className="text-volt text-xs font-display uppercase tracking-widest mb-2">
...
function MatchCard({ match }: { match: any }) {
  ...
  return (
    <div className={`card border ${isDisputed ? 'border-red-800/50' : isConfirmed ? 'border-green-800/30' : 'border-pitch-border'}`}>
      <div className="flex items-center gap-2">
        <div className="flex-1 min-w-0">
          <TeamRow
            name={match.team_a?.name ?? 'TBD'}
            isWinner={match.winner_id === match.team_a?.id}
            confirmed={isConfirmed}
          />
          <div className="border-t border-pitch-border/50 my-1.5" />
          <TeamRow
            name={match.team_b?.name ?? 'TBD'}
            isWinner={match.winner_id === match.team_b?.id}
            confirmed={isConfirmed}
          />
        </div>
        <div className="flex-shrink-0">
          <span className={`badge ${isDisputed ? 'badge-disputed' : isConfirmed ? 'badge-confirmed' : 'badge-upcoming'}`}>
...
function TeamRow({ name, isWinner, confirmed }: { name: string; isWinner: boolean; confirmed: boolean }) {
  return (
    <div className="flex items-center justify-between">
      <span className={`font-display text-sm ${isWinner && confirmed ? 'text-volt' : 'text-chalk'}`}>
        {name}
      </span>
      {isWinner && confirmed && <Trophy size={12} className="text-volt" />}
    </div>
  )
}
